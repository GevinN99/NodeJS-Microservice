...
router.post("/addUser", async (req, res) => {
    try {
        ...
        // Prepare data for creating an order in the Order Placement microservice
        const orderData = {
            orderId: `ORDER_${userId}`, // Unique order ID, you can customize this
            customerId: userId,
            amount: 100, // Specify the order amount as needed
            status: "PROCESSING",
            txnId: `TXN_${userId}`, // Unique transaction ID, you can customize this
            createdAt: new Date(),
            updatedAt: new Date(),
        };

        // Make a POST request to the Order Placement microservice to create an order
